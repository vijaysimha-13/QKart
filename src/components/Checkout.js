  ...
  const performCheckout = async (token, items, addresses) => {
    if (validateRequest(items, addresses)) {
      try {
        ...
        localStorage.setItem('balance', balance - totalCartValue);
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              {addresses.all.length === 0 ? (
                ...
                addresses.all.map((element, index) => {
                  return (
                    <Box
                      className={(element._id === addresses.selected) ? "address-item selected" : "address-item"}
                      key={index}
                    >
                      <Box onClick={() => { setAddresses({ ...addresses, selected: element._id }) }}> {element.address}</Box>
                      <Button
                        variant="text"
                        startIcon={<Delete />}
                        onClick={() => { deleteAddress(token, element._id) }}>
                        Delete
                      </Button>
                    </Box>
                  );
